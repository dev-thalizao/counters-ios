...
final class IncrementCounterFromAPIUseCaseTests: XCTestCase {
    ...
    func testIncrementFailsOnRequestError() {
    ...
    func testIncrementSucceedsOnSuccessfulRequest() {
        let (sut, client) = makeSUT()
    ...
    ) -> (sut: RemoteCounterIncrementer, client: HTTPClientSpy) {
        let client = HTTPClientSpy()
        let sut = RemoteCounterIncrementer(client: client)
    ...
    private func expect(
        _ sut: RemoteCounterIncrementer,
        ...
        sut.increment("expect-id") { receivedResult = $0 }
    ...
    private func makeNonEmptyCountersJson() -> Data {
        let json = [
            ["id": "expect-id", "title": "expect-title", "count": 1],
