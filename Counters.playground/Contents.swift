...
class MockCounterLoader: CounterLoader {
    
    func load(completion: @escaping (CounterLoader.Result) -> Void) {
        completion(.success([
            .init(id: "asdasd", title: "Coffee", count: 0)
        ]))
    }
}

let countersVC = CounterUIComposer.counterComposedWith(
    counterLoader: MockCounterLoader()
//    counterLoader: MainQueueDispatchDecorator(decoratee: MockCounterLoader())
)
